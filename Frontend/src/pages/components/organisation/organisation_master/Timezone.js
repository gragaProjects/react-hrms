...
const TimeZone = () => {
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(`/timezone/update/${formData._id}`, formData);
      toast.success('Updated successfully', { position: "top-right" });
      setDataTableData(dataTableData.map(item => {
        if (item._id === formData._id) {
          return formData; // Update the data for the edited time zone
        }
        return item;
      }));
      resetForm();
    ...
    <BlockBetween>
      ...
      <Modal isOpen={view.add || view.edit} toggle={() => onFormCancel()} className="modal-dialog-centered" size="xl">
        <ModalBody>
          ...
          <div className="p-2">
            <h5 className="title">{view.edit ? 'Update' : 'Add'} Time Zone</h5>
            <div className="mt-4">
