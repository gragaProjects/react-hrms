...
const State = () => {
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(`/state/update/${formData._id}`, formData);
      toast.success('State updated successfully', { position: "top-right" });

      // Update the dataTableData state with the updated state data
      setDataTableData(dataTableData.map(item => {
        if (item._id === formData._id) {
          return formData; // Update the data for the edited state
        }
        return item;
      }));
    ...
    <Content>
      ...
      <Modal isOpen={view.add || view.edit} toggle={() => onFormCancel()} className="modal-dialog-centered" size="xl">
        <ModalBody>
          ...
          <div className="p-2">
            <h5 className="title">{view.edit ? 'Update' : 'Add'} State</h5>
            <div className="mt-4">
                  ...
                  <Col md="6">
                    <div className="form-group">
                      <label className="form-label" htmlFor="stateName">
                        State Name <span  className="error">*</span>
                      </label>
                      <div className="form-control-wrap">
                        <input
                          type="text"
                        ...
                        />
                        {errors.stateName && <span className="invalid">{errors.stateName.message}</span>}
                      </div>
                    </div>
                  </Col>
                  <Col size="12">
