...
const Allowance = () => {
  ...
  const [formData, setFormData] = useState({
    allowance: '',
    overtime_status: 0 // Default value for overtime_status
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isValid) {
        const response = await axios.post('/allowance/add', formData);
        resetForm();
        setDataTableData([...dataTableData, response.data]);
        toast.success('Allowance added successfully', { position: "top-right" });
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(`/allowance/update/${formData._id}`, formData);
      toast.success('Allowance updated successfully', { position: "top-right" });
  ...
  const deleteAllowance = async (id) => {
    try {
      ...
      toast.success('Allowance deleted successfully', { position: "top-right" });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/allowance/get');
  ...
  const dataTableColumns = [
    ...
    {
      name: 'Is CTO / Overtime?',
      selector: (row) => row.overtime_status === 1 ? 'Yes' : 'No',
      sortable: true
    }
  ];
  ...
  return <>
    <Head title="Allowances"></Head>
    <Content>
      <BlockHead size="sm">
        <BlockBetween>
          <BlockHeadContent>
            <BlockTitle>Allowances</BlockTitle>
          </BlockHeadContent>
          <BlockHeadContent>
      ...
      <Modal isOpen={view.add || view.edit} toggle={() => onFormCancel()} className="modal-dialog-centered" size="lg">
        <ModalBody>
          ...
          <div className="p-2">
            <h5 className="title">{view.edit ? 'Update' : 'Add'} Allowance</h5>
            <div className="mt-4">
                  ...
                  <Col md="6">
                    <div className="form-group">
                      <label className="form-label" htmlFor="allowance">
                        Allowance <span  className="error">*</span>
                      </label>
                      <div className="form-control-wrap">
                        <input
                          type="text"
                          {...register('allowance', { required: "This field is required" })}
                          className="form-control"
                          value={formData.allowance}
                          onChange={(e) => setFormData({ ...formData, allowance: e.target.value })}
                        />
                        {errors.allowance && <span className="invalid">{errors.allowance.message}</span>}
                      </div>
                    </div>
                  </Col>
                  <Col md="6">
                    <div className="form-group">
                      <label className="form-label" htmlFor="overtime_status">
                        Is CTO / Overtime? <span  className="error">*</span>
                      </label>
                      <div className="form-control-wrap">
                        <select
                          {...register('overtime_status', { required: "This field is required" })}
                          className="form-select"
                          value={formData.overtime_status}
                          onChange={(e) => setFormData({ ...formData, overtime_status: e.target.value })}
                        >
                          <option value={1}>Yes</option>
                          <option value={0}>No</option>
                        </select>
                        {errors.overtime_status && <span className="invalid">{errors.overtime_status.message}</span>}
                      </div>
                    </div>
                  </Col>
                  <Col size="12">
