...
const Deduction = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isValid) {
        const response = await axios.post('/deduction/add', formData);
        resetForm();
        setDataTableData([...dataTableData, response.data]);
        toast.success('Deduction added successfully', { position: "top-right" });
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(`/deduction/update/${formData._id}`, formData);
      toast.success('Deduction updated successfully', { position: "top-right" });
  ...
  const deleteDeduction = async (id) => {
    try {
      ...
      toast.success('Deduction deleted successfully', { position: "top-right" });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/deduction/get');
    ...
    <Content>
      ...
      <Modal isOpen={view.add || view.edit} toggle={() => onFormCancel()} className="modal-dialog-centered" size="lg">
        <ModalBody>
          ...
          <div className="p-2">
            <h5 className="title">{view.edit ? 'Update' : 'Add'} Deduction</h5>
            <div className="mt-4">
                  ...
                  <Col md="6">
                    <div className="form-group">
                      <label className="form-label" htmlFor="deduction">
                        Deduction <span  className="error">*</span>
                      </label>
                      <div className="form-control-wrap">
                        <input
                          type="text"
                          {...register('deduction', { required: "This field is required" })}
                          className="form-control"
                          value={formData.deduction}
                          onChange={(e) => setFormData({ ...formData, deduction: e.target.value })}
                        />
                        {errors.deduction && <span className="invalid">{errors.deduction.message}</span>}
                      </div>
                    </div>
                  </Col>
                  <Col size="12">
